...
$ycKitCAfterSangCMs = 330     # 450 -> 360 -> 330 (2026-09-05, -30ms per user: switch to Yama earlier after Kit C; per user: Kit C fires sooner than assumed, swap to Yama moderately sooner)
...
$ycKitCAfterGodCMs = 530     # 550 -> 700 -> 560 -> 530 (2026-09-05, last step -30ms per user: switch to Yama earlier after Kit C). 700 was set when Kit C was skipping at 550 - but that was with the OLD window meaning. Under the swap-as-soon-as-fired model this window only has to outlast Godhuman C's animation until Kit C FIRES, then we swap; per user Kit C fires sooner than assumed. Raise back toward 700 only if Kit C itself gets skipped.
...
$ycKitCAfterGodCTapMs = 530   # 700 -> 560 -> 530 (2026-09-05, same reason as the held knob above; tap animation is if anything shorter)
...
$ycKitFWindowMs = 280      # 300 -> 280 (2026-09-05, per user: all delays after Kit Z "a little little bit" shorter)
# Every step AFTER Kit Z in F1 / F2 / E claw (Kit X, style X, style Z/X) and
# after Kit Z in F3 (Sang X). 260 global -> 240 (2026-09-05, per user: "a
# little little bit"). Own knob so the global stays 260 for the few steps
# that are not downstream of Kit Z (F3's Kit X after Kit C, Sang Z).
$ycAfterKitZWindowMs = 240
...
$ycYamaXWindowMs = 640     # 800 -> 640 (2026-09-05, per user: "Yama fires faster than you think, switch earlier" - same ~20% cut Kit C got)
...
$ycKitZAfterYamaXMs = 600  # 750 -> 600 (2026-09-05, per user: "the same applies to Kit Z" - fires sooner than assumed, swap onward earlier)
...
$Combo_Godhuman_Tail = @(
    ...
    @('swap', $slotFruit),  @('spam', $keyZ, $ycKitZAfterYamaXMs, $ycSpamIntervalMs), @('spam', $keyX, $ycAfterKitZWindowMs, $ycSpamIntervalMs),   # Kitsune Z (own window after Yama X), X (after-Kit-Z window)
    @('swap', $slotFightStyle), @('spam', $keyX, $ycAfterKitZWindowMs, $ycSpamIntervalMs),      # Godhuman X
    @('swap', $slotFruit),  @('spam', $keyF, $ycKitFWindowMs, $ycSpamIntervalMs),   # Kitsune F - own window
    @('swap', $slotFightStyle), @('spam', $keyZ, $ycAfterKitZWindowMs, $ycSpamIntervalMs)       # Godhuman Z
)
...
$Combo_Sanguine = @(
    ...
    @('swap', $slotSword),  @('spam', $keyX, $ycYamaXWindowMs, $ycSpamIntervalMs),   # Yama X - own window, see $ycYamaXWindowMs
    @('swap', $slotFruit),  @('spam', $keyZ, $ycKitZAfterYamaXMs, $ycSpamIntervalMs), @('spam', $keyX, $ycAfterKitZWindowMs, $ycSpamIntervalMs),   # Kitsune Z (own window after Yama X), X (after-Kit-Z window)
    @('swap', $slotFightStyle), @('spam', $keyZ, $ycAfterKitZWindowMs, $ycSpamIntervalMs),      # Sanguine Z
    @('swap', $slotFruit),  @('spam', $keyF, $ycKitFWindowMs, $ycSpamIntervalMs),   # Kitsune F - own window
    @('swap', $slotFightStyle), @('spam', $keyX, $ycAfterKitZWindowMs, $ycSpamIntervalMs)       # Sanguine X
)
...
$Combo_SanguineAlt = @(
    ...
    @('swap', $slotFruit),  @('spam', $keyF, $ycKitFWindowMs, $ycSpamIntervalMs), @('spam', $keyZ),   # Kitsune F (own window), Z
    @('swap', $slotFightStyle), @('spam', $keyX, $ycAfterKitZWindowMs, $ycSpamIntervalMs)       # Sanguine X (after-Kit-Z window)
)
...
$Combo_EClaw = @(
    ...
    @('swap', $slotSword),  @('spam', $keyX, $ycYamaXWindowMs, $ycSpamIntervalMs),   # Yama X - own window, see $ycYamaXWindowMs
    @('swap', $slotFruit),  @('spam', $keyZ, $ycKitZAfterYamaXMs, $ycSpamIntervalMs), @('spam', $keyX, $ycAfterKitZWindowMs, $ycSpamIntervalMs),   # Kitsune Z (own window after Yama X), X (after-Kit-Z window)
    @('swap', $slotFightStyle), @('spam', $keyX, $ycAfterKitZWindowMs, $ycSpamIntervalMs),      # E claw X
    @('swap', $slotFruit),  @('spam', $keyF, $ycKitFWindowMs, $ycSpamIntervalMs),   # Kitsune F - own window
    @('swap', $slotFightStyle), @('spam', $keyZ, $ycAfterKitZWindowMs, $ycSpamIntervalMs)       # E claw Z
)
